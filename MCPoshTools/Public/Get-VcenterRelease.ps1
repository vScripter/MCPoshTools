function Get-VcenterRelease {

    <#
    .SYNOPSIS
        Return information about vCenter Server release information
    .DESCRIPTION
        Return information about vCenter Server release information

        Information is pulled from an external source, which is maintained by an unofficial 3rd party from VMware (http://www.virten.net/repo/vcenterReleases.json)
    .PARAMETER InputPath
    ...
    param (
        ...
        $InputPath = "$PSScriptRoot\..\Inputs\vcenterReleases.json"
    )
    ...
    PROCESS {
        ...
        try {
            ...
            foreach ($entry in $json.data.vcenterReleases) {
...
} # end function Get-VcenterRelease
