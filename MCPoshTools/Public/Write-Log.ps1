function Write-Log {
    ...
    .DESCRIPTION
        ...
        to: C:\Users\%USERNAME%\AppData\Roaming. The file will be named 'PowerShell_Write-Log_Output_yyyyMMdd.log'; where 'yyyyMMdd' represends a four character year,
    ...
    param (
        ...
        $LogFile = "$($ENV:APPDATA)\PowerShell_Write-Log_Output_$($(Get-Date).ToString('yyyyMMdd')).log",

        [parameter(Position = 2)]
        [validatepattern('[0-8]')]
        $Mode = 3,

        [Switch]$InternalOutput
    )
    ...
    BEGIN {
        ...
        if (-not (Test-Path -LiteralPath (Split-Path -LiteralPath $LogFile) -PathType Container)) {
            ...
            Write-Warning -Message "[Write-Log] Specified log path not found; using default path"
            $defaultLogPath = "$($ENV:APPDATA)\PowerShell_Write-Log_Output_$($(Get-Date).ToString('yyyyMMdd')).log"
        ...
        if ($Global:LogMode -match $modeRangeRegEx) {
            ...
            if ($InternalOutput) { Write-Verbose -Message "[Write-Log] Global Log Mode found. Setting mode to {$($Global:LogMode)}" }

            $Mode = $Global:LogMode

        } elseif ($Global:LogMode -and $Global:LogMode -notmatch $modeRangeRegEx) {

            Write-Warning -Message "[Write-Log] Global Log Mode found, but the value assigned is not a valid mode {$($Global:LogMode)}. Using default/selected Mode {$Mode}"
        ...
        if ($Global:LogFile -and (Test-Path -LiteralPath (Split-Path $Global:LogFile) -PathType Container)) {
            ...
            if ($InternalOutput) { Write-Verbose -Message "[Write-Log] Global Log File found. Setting log file to {$($Global:LogFile)}" }

            $LogFile = $Global:LogFile

        } elseif ($Global:LogFile -and (-not (Test-Path -LiteralPath (Split-Path $Global:LogFile) -PathType Container))) {

            Write-Warning -Message "[Write-Log] Global Log file found, but I can't access it at {$($Global:LogFile)}. Using default/selected LogFile {$LogFile}"
    ...
    PROCESS {
        ...
        switch ($Mode) {
            ...
            0 {
                ...
                # Only log to file
                if ($InternalOutput) { Write-Verbose -Message "[Write-Log][To File] Writing message {$messageString} to log file {$((Resolve-Path $LogFile).Path)}" }
                ...
                try {
                    ...
                    Write-Warning -Message "[Write-Log][ERROR] Could not write to log file {$LogFile}"
    ...
    END {
        ...
        if ($InternalOutput) { Write-Verbose -Message "[Write-Log] Processing Complete" }
